...
export default async function syncDataAndAssets() {
    ...
    const nextCache = path.join(rootPath, 'next-cache');
    ...
    try {
        ...
        if (fs.existsSync(nextCache)) {
            if (fs.existsSync(path.join(nextCache, 'fetch-cache'))) {
                await fsPromises.rm(path.join(nextCache, 'fetch-cache'), { recursive: true, force: true });
            }
            if (fs.existsSync(path.join(nextCache, 'images'))) {
                await fsPromises.rm(path.join(nextCache, 'images'), { recursive: true, force: true });
            }
        } else {
            await fsPromises.mkdir(nextCache);
        }
...
function syncRemoteAssets() {
    return new Promise((resolve, reject) => {
        ...
        console.log(chalk.dim(`Source: ${process.env.REMOTE_USER}@${process.env.REMOTE_ADDR}:${process.env.REMOTE_DIR}/directus/uploads/`));
        console.log(chalk.dim(`Destination: ${path.join(rootPath, 'directus/uploads/')}\n`));
        ...
        rsync.set('progress').unset('quiet'); // show progress
        rsync.set('ignore-existing'); // ignore files that already exist in destination

        rsync.output(
            function (data) {
                logUpdate(chalk.dim(data.toString()));
            }, function (data) {
